...
def get_clean_filing_text(cik, ticker = '', year_start = 2000, year_end = 2015):
    ...
    #cur.execute('select f.cik, f.date_filed, f.form_type, f.filing_id, f.filing from filings f where f.cik = %s;',  (cik, ))
    ...
    for record in cur:
            ...
            if (form_type in settings.FORM_TYPES and 
                ...
                if os.path.isfile(settings.BASE_PATH_CLEAN_FILINGS + filename_clean):
                    print('Filing {} already exists. Skipping.'.format(filename_clean))
                else:
                    # read file and clean it
                    f_clean = open(settings.BASE_PATH_CLEAN_FILINGS + filename_clean, 'w')
                    try:
                        print('Cleaning file {} and writing to {}'.format(filename, filename_clean))
                        #f = open(settings.BASE_PATH_CLEAN_FILINGS + filename, 'r')
                        #filing_text = f.read()
                        #f.close()
                        filing_text = filing
                        clean_text = clean_filing(filing_text)                        
                ...
                sentences = doc2vec.LabeledLineSentence(settings.BASE_PATH_CLEAN_FILINGS + filename_clean)
...
#get_clean_filing_text('1137774', 'PRU', 2000, 2015)
